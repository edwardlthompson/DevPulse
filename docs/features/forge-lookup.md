...
| `GitHubScan.toOffer` | Hint-first: if a `GithubHint` (F-Droid `sourceCode` ∪ persisted map ∪ **shipped `github-repos/verified.tsv`** ∪ `io.github.Owner.Repo`) is set, **list immediately** and `GET /repos/{owner}/{repo}/releases` for tag, date, and APK. `searchRepos` stays at 0. HTTP failure keeps the hint listing (F-Droid version/date if any). Bound APKs may omit the package id in the filename (`GitHubReleasePick.bound`). Success → persist. No hint + leftover GitLab/Codeberg `sourceCode` → list leftover with zero search HTTP. Fresh known miss (7-day TTL) skips search when `ListingMiss.Searched`. Refresh: another store **listed** → skip name-search (`github $pkg skip search (listed)`). Play **miss** is not a listing. Store-less leftovers name-search GitHub only when Settings `searchUnknowns` is on, or in the post-update `GitHubDiscover` worker (10/min anonymous) + release verify (`GitHubReleasePick.leftover` walks to an APK). Empty leftover search is a 30-day skip in the worker (`GithubSearchSkip`). A GitHub `Searched` miss is also the GitLab/Codeberg answer — no leftover search HTTP. Do not skip remaining leftover apps after 403; wait and continue unless the user taps Stop |
### Token storage
...
- ✅ User-visible behavior: GitHub **hint-first** (F-Droid `sourceCode` on the same app object ∪ persisted map ∪ `io.github.Owner.Repo`) lists without `searchRepos`. `listReleases` fills tag/APK so Has update matches GitHub, not a stale F-Droid `suggestedVersionName`. Store-less leftovers always name-search (10/min, no token). `forge_lookup_search_unknowns` stays a Settings string; leftover search no longer waits on it. When search runs, list if a release has the package **or** a slug-matched repo has an APK
...
3. _Then_ the app is listed, `searchRepos` stays at 0, `listReleases` is 1, and the offer uses the GitHub `tag_name` / `published_at` when an APK asset exists
