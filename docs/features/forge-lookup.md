...
- ✅ User-visible behavior: GitHub **hint-first** (F-Droid `sourceCode` on the same app object ∪ persisted map) lists without `searchRepos`. `listReleases` fills tag/APK so Has update matches GitHub, not a stale F-Droid `suggestedVersionName`. Leftover name search is **opt-in** (`forge_lookup_search_unknowns`, default off). When search is on, list only if a release asset/title/tag/body contains the full package (or `-`/`_` variant)
