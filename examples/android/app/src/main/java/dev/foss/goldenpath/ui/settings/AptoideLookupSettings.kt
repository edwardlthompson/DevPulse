...
fun AptoideLookupSettings(modifier: Modifier = Modifier) {
    ...
    val enabled by prefs.aptoideLookupEnabled.collectAsStateWithLifecycle(false)
    Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(SpacingMd)) {
        Text(text = stringResource(R.string.aptoide_title), style = MaterialTheme.typography.titleMedium)
        ...
        PreferenceSwitch(
            ...
            checked = enabled,
            onCheckedChange = { value -> scope.launch { prefs.setAptoideLookupEnabled(value) } },
        )
    }
}
